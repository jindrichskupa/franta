...
const helpText = `franta — keys
  ...
  I                   cluster info: brokers + controller (left) and the
                      selected topic's partition leader/replicas/ISR (right);
                      tab/1/2 switch pane, r reload, esc back
...
func (m Model) normalFooter() string {
	...
	switch m.paneFocus {
	case paneTopics:
		hint = "↑/↓ nav  enter switch  / search  o sort  i internal  r reload  •  n new  D del  a parts  c config  •  1/2/3 t tab focus  •  space pause  •  s p P g e  •  C cluster  •  I info  •  S schemas  •  ? help  q quit"
	case paneDetail:
		if !m.detailRaw && m.detailTree != nil {
			hint = "↑/↓ nav  enter fold  ←/→ collapse/expand  home/end top/bottom  •  v raw  •  y/Y/ctrl+y copy  •  1/2/3 t tab focus  •  ? help  q quit"
		} else {
			hint = "↑/↓/pgup/pgdn scroll  •  v tree  •  y/Y/ctrl+y copy  •  1/2/3 t tab focus  •  P produce-template  •  space pause  •  s p g e  •  C cluster  •  I info  •  S schemas  •  ? help  q quit"
		}
	default:
		hint = "↑/↓ nav  / f filter (e.g. header['src'] == \"web\")  •  P produce-template  •  y/Y/ctrl+y copy  •  1/2/3 t tab focus  •  space pause  •  s p g e  •  C cluster  •  I info  •  S schemas  •  ? help  q quit"
	}
	prefix := ""
	if m.paused {
		prefix = "[PAUSED] "
